...
#returns a cursor to action on the Db
def get_cursor():
...
#this is the user id being used for testing, in future this will have to come from a form/request parameter
user_id = 1
...
#The Flask bits here are only for testing purposes, to see a live version of the charts
@application.route("/")
def index():
    wpm = wpmChart()
    acc = accChart()
    wpmChartJSON = wpm.get()
    accChartJSON = acc.get()
    #variables inside index2.html are set in the arguments here
    return render_template("index2.html", wpmChartJSON = wpmChartJSON, accChartJSON = accChartJSON)
