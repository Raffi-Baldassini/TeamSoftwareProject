...
class DB:
    ...
    def upload_game(game_stats):
        ...
        if len(cursor.fetchall()) == 0:
            cursor.execute(
                """INSERT INTO stats(id, solo_games, online_games,
            ...
            cursor.execute(
                """UPDATE stats SET solo_games = %s, words = %s, chars = %s,
        ...
        cursor.execute(
            "SELECT * FROM records WHERE id = %s AND score_date = %s",
            (user, today))
        if len(cursor.fetchall()) == 0:
            cursor.execute(
                """INSERT INTO records(id, score_date, wpm, acc)
                                VALUES (%s, %s, %s,%s);""",
                (user, today, wpm, acc))
        else:
            cursor.execute(
                "SELECT wpm,acc FROM records WHERE id = %s AND score_date = %s",
                (user, today))
            ...
            if current_wpm > wpm:
                ...
                if current_acc > acc:
                    ...
                    day_best = (wpm, acc)
            cursor.execute(
                """UPDATE records SET wpm = %s, acc = %s WHERE id = %s AND score_date = %s""",
    ...
    def getBestWPM(user):
        ...
        cursor.execute("SELECT wpm FROM records WHERE id=%s AND score_date=%s",
                       (user, today))
        ans = cursor.fetchall()
        if len(ans) != 0:
            wpm_day = ans[0][0]
